...
fn main(){
    ...
    let n:usize = r.readl();
    let h_s:Vec<i64> = r.read_vec();

    let mut dp = vec![std::u64::MAX;n];
    dp[0] = 0;
    dp[1] = (h_s[1] - h_s[0]).abs() as u64;

    for i in 2..n {
        if( (dp[i - 2]) + ((h_s[i] - h_s[i - 2]).abs() as u64)  ) < ( (dp[i - 1]) + ((h_s[i] - h_s[i - 1]).abs() as u64)  ) {
            dp[i] = (dp[i - 2]) + ((h_s[i] - h_s[i - 2]).abs() as u64);
        }else{
            dp[i] = ( (dp[i - 1]) + ((h_s[i] - h_s[i - 1]).abs() as u64)  );
        }
    }
